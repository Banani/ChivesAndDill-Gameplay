import { ChannelSubSpell } from '@bananos/types';
...
export const spellsReducer = (state: SpellsState = initialState, action: SpellsAction): SpellsState => {
    switch (action.type) {
        ...
        case SpellsActionTypes.ADD_ACTIVE_SPELL_CAST: {
            ...
            if ((action.payload.event.spell as ChannelSubSpell).channelTime) {
                activeSpellsCasts = {
                    ...activeSpellsCasts,
                    [action.payload.event.casterId]: {
                        castTime: (action.payload.event.spell as ChannelSubSpell).channelTime,
