...
export const QuestDescription: FunctionComponent<QuestDescriptionProps> = ({ questSchema }) => {
   ...
   const items = useMemo(
      () =>
         map(questSchema.questReward.items, (item, index) => {
            const itemTemplate = itemTemplates[item.itemTemplateId];
            if (itemTemplate) {
               return (
                  <div className={styles.ItemContainer} key={index}>
                     <ItemPreview itemTemplate={itemTemplate} showMoney={false} highlight={ItemPreviewHighlight.none} />
                  </div>
               );
            }
         }),
   ...
   const renderQuests = _.map(questProgress, (currentQuestProgress, questIndex) => {
      const questStage = questSchema.stages[currentQuestProgress.activeStage];

      return (
         <div className={styles.questDesc} key={questIndex}>
            {_.map(questStage?.stageParts, (stagePart, stagePartId) => {
               return (
                  <div
                     key={stagePartId}
                     className={currentQuestProgress.stagesProgress[currentQuestProgress.activeStage][stagePartId].isDone ? styles.stagePartDone : ''}
                  >
                     <QuestStagePart
