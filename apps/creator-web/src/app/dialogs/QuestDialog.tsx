               ...
               <Autocomplete
                  ...
                  onChange={(_, element) => {
                     if (element) {
                        setQuest({
                           ...quest,
                           questReward: { ...quest.questReward, items: [...(quest.questReward.items ?? []), { itemTemplateId: element.id, amount: 1 }] },
                        });
                     }
                  }}
                  options={_.map(
                     _.filter(itemTemplates, (item) => !quest.questReward.items?.find((rewardItem) => rewardItem.itemTemplateId === item.id)),
               ...
               <DataGrid
                  ...
                  processRowUpdate={(newRow: QuestRewardItem) => {
                     setQuest({
                        ...
                        questReward: {
                           ...quest.questReward,
                           items: quest.questReward.items?.map((item) => {
                              ...
                              <GridActionsCellItem
                                 ...
                                 onClick={() => {
                                    setQuest({
                                       ...
                                       questReward: {
                                          ...quest.questReward,
                                          items: quest.questReward.items?.filter((item) => item.itemTemplateId !== id),
                                       },
                                    });
                                 }}
                              />,
