...
export class MonsterMovementEngine extends Engine {
    ...
    none = (monster: Monster) => {
        ...
        if (distanceBetweenTwoPoints(monster.location, respawn.location) <= monster.movementSpeed) {
            ...
            const offset = {
                x: Math.min(monster.movementSpeed / 2, Math.abs(xDistance)) * Math.sign(xDistance),
                y: Math.min(monster.movementSpeed / 2, Math.abs(yDistance)) * Math.sign(yDistance),
            };
            const newLocation = {
    ...
    patrol = (monster: Monster) => {
        ...
        if (distanceBetweenTwoPoints(monster.location, directionPatrolPoint) <= monster.movementSpeed) {
            ...
            const offset = {
                x: Math.min(monster.movementSpeed / 4, Math.abs(xDistance)) * Math.sign(xDistance),
                y: Math.min(monster.movementSpeed / 4, Math.abs(yDistance)) * Math.sign(yDistance),
            };
            newLocation = {
                x: monster.location.x + offset.x,
                y: monster.location.y + offset.y,
    ...
    chasing(monster: Monster, targetId: string) {
        ...
        if (distanceBetweenTwoPoints(allCharacters[targetId].location, monster.location) <= monsterTemplate.desiredRange) {
            this.eventCrator.createEvent<PlayerStopedAllMovementVectorsEvent>({
                type: EngineEvents.PlayerStopedAllMovementVectors,
                characterId: monster.id,
            });
        ...
        if (direction) {
            ...
            const offset = {
                x: Math.min(monster.movementSpeed, Math.abs(xDistance)) * Math.sign(xDistance),
                y: Math.min(monster.movementSpeed, Math.abs(yDistance)) * Math.sign(yDistance),
            };
