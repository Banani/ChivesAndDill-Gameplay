...
export function getCrossingPointsWithWalls(movementSegment, areas) {
    return areas.reduce((prev, polygon) => {
        const intersections = [];
...
export function isSegementCrossingWithAnyWall(movementSegment, areas) {

    for (let polygon of areas) {
        for (let i = 0; i < polygon.length; i++) {
            const crossPoint = getSegmentsCrossingPoint(movementSegment, [polygon[i], polygon[(i + 1) % polygon.length]]);
            if (crossPoint !== null) {
                return true;
